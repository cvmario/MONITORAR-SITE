...
/* =========================================================================
 * FIRESTORE HELPER
 * (usa a API nativa do firebase-admin: db.collection(...).add()/.get()/.where())
 * ========================================================================= */
...
class FirestoreHelper {
  ...
  async saveFinding(finding) {
    if (!db) return null;
    try {
      const ref = await db.collection('findings').add({
        ...finding,
        timestamp: new Date().toISOString(),
      });
      return ref.id;
  ...
  async getFindingsByKeyword(keyword) {
    if (!db) return [];
    try {
      const snapshot = await db.collection('findings').where('keyword', '==', keyword).get();
